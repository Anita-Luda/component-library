...
const Atoms = {
    ...
    _getSnippet(id, fallbackFn, p) {
        // Favor contextual snippets for inline tags if they exist
        const contextId = \`html5-\${id.split('-')[1]}-context\`;
    ...
    _wrap: (tag, p, extraAttr = '') => {
        const classes = \`cmp ${tag} type-\${p.type||'default'} state-\${p.state||'default'} \${p.state && p.state!=='default'?'force-'+p.state:''} \${p.classes||''} is-atom\`;
    ...
    image: (p = {}) => Atoms.img(p),
    input_raw: (p = {}) => Atoms._getSnippet(\`atoms-input_raw-\${p.type||'default'}-\${p.state||'default'}\`, (p) => Atoms._wrap('input', p), p),
    ...
    strike: (p = {}) => Atoms._getSnippet('atoms-s-default-default', (p) => Atoms._wrap('strike', p), p),
    ...
    toggle_raw: (p = {}) => Atoms._getSnippet('atoms-toggle_raw-default-default', (p) => Atoms._wrap('input', p, 'type="checkbox" role="switch"'), p),
    range_raw: (p = {}) => Atoms._getSnippet('atoms-range_raw-default-default', (p) => Atoms._wrap('input', p, 'type="range"'), p)
};

window.Atoms = Atoms;
if (typeof module !== 'undefined') module.exports = Atoms;

// Missing methods identified in code review
Atoms.icon_button = (p = {}) => Atoms._getSnippet(`atoms-icon_button-${p.type||'primary'}-${p.state||'default'}`, (p) => Atoms._wrap('button', p, 'aria-label="Przycisk"'), p);
Atoms.radio = (p = {}) => Atoms._getSnippet(`atoms-radio-default-${p.state||'default'}`, (p) => Atoms._wrap('input', p, 'type="radio"'), p);
Atoms.checkbox = (p = {}) => Atoms._getSnippet(`atoms-checkbox-default-${p.state||'default'}`, (p) => Atoms._wrap('input', p, 'type="checkbox"'), p);
