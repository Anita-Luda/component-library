...
export const Atoms = {
    ...
    _getSnippet(blueprint, type, state, content = '') {
        const slug = blueprint.replace('atoms.', '');
        const targetId = `atoms-${slug}-${type}-${state}`;

        // Try exact match
        let el = this._catalog.getElementById(targetId);

        // Fallback sequence
        if (!el) el = this._catalog.getElementById(`atoms-${slug}-${type}-default`);
        if (!el) el = this._catalog.getElementById(`atoms-${slug}-default-${state}`);
        if (!el) el = this._catalog.getElementById(`atoms-${slug}-default-default`);

        if (!el) {
            console.warn(`Atom not found: ${targetId} (and no fallback)`);
            return `<span class="error-missing">Missing Atom: ${slug}</span>`;
        }
        ...
        html = html.replaceAll('{{SRC}}', `https://picsum.photos/seed/${slug}/200`);
