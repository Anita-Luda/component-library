...
const Molecules = {
    ...
    card: (p) => {
        ...
        return `<article class="cmp card type-${p.type||'default'} state-${p.state||'default'} ${p.state && p.state!=='default'?'force-'+p.state:''} is-molecule">${h}${b}${f}</article>`;
    },
    table: (p) => {
    ...
    form_field: (p) => {
        ...
        if (p.inputType === 'select') {
            ...
            input = `<select id="${id}" class="cmp select-raw state-${p.state||'default'} is-atom">${opts}</select>`;
        } else if (p.multiline) {
            input = Atoms.textarea_raw({ id, ...p });
        } else {
            input = Atoms.input_raw({ id, ...p });
        }
        return `<div class="cmp form-field state-${p.state||'default'} is-molecule">${label}${input}</div>`;
    },

    choice_field: (p) => {
        const id = `choice-${Math.random().toString(36).substr(2, 5)}`;
        const input = p.isRadio ? Atoms.radio({ id, ...p }) : Atoms.checkbox({ id, ...p });
        const label = Atoms.label({ content: p.label, for: id });
        return `<div class="cmp choice-field state-${p.state||'default'} is-molecule">${input}${label}</div>`;
    },
    ...
    tabs: (p) => {
        const items = (p.items||['Tab 1', 'Tab 2', 'Tab 3']).map((it, i) => `<button role="tab" aria-selected="${i===0?'true':'false'}" class="cmp tab-btn ${i===0?'is-active':''} is-atom">${it}</button>`).join('');
        return `<div role="tablist" class="cmp tabs is-molecule">${items}</div>`;
    },
    ...
    tree_item: (p) => {
        ...
        return `<details class="cmp tree-item is-molecule" ${depth===0?'open':''}>
    ...
    mega_menu_column: (p) => {
        ...
        return `<div class="cmp mega-menu-column is-molecule">${head}<ul>${items}</ul></div>`;
    },
    ...
    recursive_list_item: (p) => {
        ...
        return `<span>${p.content}</span>${sub}`;
    },
    ...
    product_card: (p) => {
        ...
        return `<article class="cmp product-card is-molecule">${img}${title}${price}${btn}</article>`;
    },
    color_swatches: (p) => {
        ...
        return `<div class="cmp color-swatches is-molecule">${label}${swatches}</div>`;
    },
    ...
    usage_meter: (p) => {
        ...
        return `<div class="cmp usage-meter is-molecule">${label}${val}${bar}</div>`;
    },
    integration_tile: (p) => {
        ...
        return `<div class="cmp integration-tile is-molecule">${icon}${name}${status}</div>`;
    },
    filter_pill: (p) => {
        const text = Atoms.span({ content: p.content });
        const close = Atoms.button({ content: '×', type: 'tertiary', classes: 'pill-close' });
        return `<span class="cmp filter-pill is-molecule">${text}${close}</span>`;
    },
    ...
    alert: (p) => {
        ...
        const text = Atoms.span({ content: p.content });
        return `<div class="cmp alert type-${p.type||'default'} state-${p.state||'default'} is-molecule" role="alert">${icon}${text}</div>`;
    },
    statistic: (p) => {
        ...
        return `<div class="cmp statistic is-molecule">${lab}${val}${trend}</div>`;
    },
    modal: (p) => {
        ...
        return `<div class="cmp modal-overlay is-molecule" role="dialog"><div class="modal-content">${h}${b}${close}</div></div>`;
    },
    toast: (p) => {
        const icon = Atoms.icon({ name: 'bell' });
        const text = Atoms.span({ content: p.content });
        return `<div class="cmp toast type-${p.type||'default'} is-molecule" role="status">${icon}${text}</div>`;
    }
};
