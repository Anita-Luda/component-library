...
const Organisms = {
    ...
    // --- TASK MANAGEMENT ---
    kanban_column: (p = {}) => {
        const h = Atoms.heading({ level: 4, content: p.title || 'Kolumna' });
        const count = Atoms.badge({ content: p.count || '0', type: 'tertiary' });
        const header = `<div class="column-header">${h}${count}</div>`;
        const tasks = (p.tasks || []).map(t => Molecules.task_card(t)).join('');
        return `<div class="cmp kanban-column ${p?.classes||''} is-organism">${header}<div class="task-list">${tasks}</div></div>`;
    },
    sprint_board: (p = {}) => {
        const cols = [
            { title: 'Do zrobienia', count: '3', tasks: [{title: 'Mielenie rzeczywistości', id: 1}, {title: 'Kwantowanie zupy', id: 2}] },
            { title: 'W trakcie', count: '1', tasks: [{title: 'Szycie czasu', id: 3}] },
            { title: 'Gotowe', count: '12', tasks: [{title: 'Naprawa wszechświata', id: 4}] }
        ].map(c => Organisms.kanban_column(c)).join('');
        return `<div class="cmp sprint-board ${p?.classes||''} is-organism profile-grid">${cols}</div>`;
    },
